...
public class HRSActivity extends BleProfileActivity implements HRSManagerCallbacks {
	...
	private int PP =1024;                      // used to find peak in pulse wave, seeded
	private int T = 1024;                     // used to find trough in pulse wave, seeded
	private int thresh = 1024;                // used to find instant moment of heart beat, seeded
	...
	private double[] XT=new double[1100];	//显示点缓存
	private double[] YT=new double[1100];
	...
	private void updateGraph(final int hrmValue) {
		runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				for (int i = mCounter-100; i < mCounter; i++) {			//丢掉前20个数据，可能显示有问题，未知bug
   ...
   private  void HeartComputer(int Signal){
	...
	if (N > 2500){                              // if 2.5 seconds go by without a beat
		thresh = 1024;                          // set thresh default
		PP = 1024;                               // set P default
		T = 1024;                               // set T default
