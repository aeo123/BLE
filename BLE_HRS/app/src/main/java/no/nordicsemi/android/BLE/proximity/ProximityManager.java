 ...
 ******************************************************************************/
package no.nordicsemi.android.BLE.proximity;
...
public class ProximityManager implements BleManager<ProximityManagerCallbacks> {
	...
	private BluetoothGattCharacteristic mAlertLevelCharacteristic, mLinklossCharacteristic, mBatteryCharacteritsic;
	...
	BluetoothGattServerCallback mGattServerCallbacks = new BluetoothGattServerCallback() {
		@Override
				...
				public void run() {
					...
					if (IMMEIDIATE_ALERT_SERVICE_UUID.equals(service.getUuid()))
						...
						if (mBluetoothGatt == null) {
							mBluetoothGatt = mDeviceToConnect.connectGatt(mContext, true, mGattCallback);
			...
			if (status == BluetoothGatt.GATT_SUCCESS) {
				...
				for (BluetoothGattService service : services) {
					if (service.getUuid().equals(IMMEIDIATE_ALERT_SERVICE_UUID)) {
						...
						mBatteryCharacteritsic = service.getCharacteristic(BATTERY_LEVEL_CHARACTERISTIC_UUID);
					}
				}
